...
4. Add responsibilities, decision framework, constraints, outputs, collaboration.
5. Add the required **Completion and handoff** block: definition of done, stop when, hand over to (next role + package), and start rule for the next role (see [references/completion-and-handoff.md](references/completion-and-handoff.md)).
6. Add optional sections only when they add signal (e.g. Escalation) or use a second file if the role is huge.
7. Self-check: length, sparse emphasis, no repo-specific URLs, completion block present.
...
- **Brevity:** default to tight bullets; avoid long prose and repetition across sections. If a role needs more than ~1–2 screens of text, split into a second file or a separate how-to skill—do not pack one agent to encyclopedia size.
